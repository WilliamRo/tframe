...
def _recursive_modify(v_dict, level=0):
  ...
  for key in list(v_dict.keys()):
    ids = re.findall(r'(?<=weights_)\d+', key)
    if not ids: continue
    assert len(ids) == 1
    id = ids[0]
    # Find mask_id
    ...
    # Now pair is found
    pairs.append((id, v_dict.pop(key), v_dict.pop(mask_key)))
  # Update v_dict
  for id, weights, mask in pairs:
    assert isinstance(weights, list) and isinstance(mask, list)
    hist_key = 'histogram_' + id
    v_dict['masked_weights_' + id] = [w * m for w, m in zip(weights, mask)]
